...
**同期のためだけなら不要。** 遠隔操作は Mac の中のファイルを直接いじるので、コピーは1つしか存在しない。同期する相手がいないため `git pull` は空振りする（`Already up to date.` と出るだけ）。

ただし git には同期とは別の価値がある。

| 目的 | 遠隔操作のみの場合 |
| --- | --- |
| ① 端末間の同期 | ❌ 不要 |
| ② バックアップ | ✅ 必要（Mac が壊れたら全部消える） |
| ③ **履歴・巻き戻し** | ✅ **最重要** |

**③が本命。** AI にコードを書かせると意図しない変更が混ざることがある。git があれば差分を確認でき、まずければ戻せる。無いと「昨日は動いていたのに」となった時点で復旧手段がない。
...
| --- | --- |
| Mac だけで作業する | 不要 |
| ブラウザの Claude Code も使う | **必要** |
| 別の PC でも編集する | **必要** |

判断に迷ったら実行しておけばよい。不要な場合でも `Already up to date.` と出るだけで、害はない。
...
編集する（Claude Code でもエディタでも）。区切りがついたら記録する。
...
| コマンド | 意味 |
| --- | --- |
| `git pull` | 他の書き手の変更を取り込む（書き手が1人なら省略可） |
| `git status` | 今どういう状態か確認する。**迷ったらまずこれ** |
| `git add -A` | 変更をまとめる |
| `git commit` | 履歴として記録する ← **③の価値はここで生まれる** |
| `git push` | GitHub へ送る ← **②の価値はここで生まれる** |

### コミットの頻度

毎回ではなく、**「ここまでは残しておきたい」と思った時点**でよい。

- 調査資料を1本書き終えた
- 機能が1つ動くようになった
- これから大きく作り変えるので、その前の状態を残したい

逆に、**AI に大きな変更を任せる直前**は必ずコミットしておく。戻れる地点を作ってから走らせるのが安全。
...
| コマンド | 用途 |
| --- | --- |
| `git log --oneline` | 過去のコミット一覧を見る |
| `git diff` | 前回コミットから何が変わったか見る |
| `git restore ファイル名` | そのファイルを前回コミット時点へ戻す |
| `git restore .` | **全ファイルを前回コミット時点へ戻す**（未コミットの変更は消える） |
